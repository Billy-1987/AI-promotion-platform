...
function WeekCard({ entry, currentWeek, onBrandClick, onRecommendClick, hasRecommend, itemBrands, itemBrandCount, weekNote, onEditNote, isHQ }: {
  ...
  // 相对当前周的状态 → 决定 entry.week 徽章配色
  // - 当前周：蓝色 #0034cc
  // - 过去周：橙色 #f97316
  // - 未来周：绿色 #16a34a
  const cmp = currentWeek ? compareWeek(entry.week, currentWeek) : 0
  const status: 'past' | 'current' | 'future' = cmp < 0 ? 'past' : cmp > 0 ? 'future' : 'current'
  const weekBadgeColor = status === 'current' ? '#0034cc' : status === 'past' ? '#f97316' : '#16a34a'
    ...
    <div className="flex flex-col md:flex-row md:gap-3 md:items-start gap-1">
      ...
      <div className="hidden md:flex md:w-20 md:shrink-0 md:pt-3 md:flex-col items-center md:gap-1.5">
        {isCurrent && currentWeek && (
          <>
            ...
            <span className="px-3 py-1.5 border text-base font-bold rounded-xl text-center" style={{ background: 'rgba(0,52,204,0.10)', borderColor: 'rgba(0,52,204,0.40)', color: '#0034cc' }}>
              {currentWeek}
            </span>
          </>
        )}
      ...
      {isCurrent && currentWeek && (
        <div className="md:hidden flex items-center gap-2">
          ...
          <span className="px-2.5 py-1 border text-sm font-bold rounded-lg text-center" style={{ background: 'rgba(0,52,204,0.10)', borderColor: 'rgba(0,52,204,0.40)', color: '#0034cc' }}>
            {currentWeek}
          </span>
        </div>
      )}
      ...
      <div className="flex-1 min-w-0 glass-card rounded-2xl overflow-hidden">
        <div className="flex flex-wrap items-center gap-2 md:gap-4 px-3 md:px-6 py-3 md:py-4 border-b border-slate-200/60">
          <span className="px-2.5 md:px-3 py-1 text-white text-sm font-bold rounded-lg whitespace-nowrap flex-shrink-0" style={{ background: weekBadgeColor }}>{entry.week}</span>
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-4xl bg-white flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {filtered.length > 0 && (
            <div className="mb-4 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-600 leading-relaxed flex items-start gap-2">
              <span className="text-slate-400 flex-shrink-0">ⓘ</span>
              <span>
                每张图下方的<span className="font-semibold text-slate-700">铺店覆盖率</span> =
                <span className="font-mono text-slate-700 mx-1">该款铺店数 ÷ 品牌铺店总数</span>。
                <span className="inline-block px-1.5 py-0.5 rounded mr-1" style={{ background: '#10b981' }}><span className="text-white">绿</span></span>
                ≥ 80% 已普铺 / 达标推荐；
                <span className="inline-block px-1.5 py-0.5 rounded mx-1" style={{ background: '#f59e0b' }}><span className="text-white">橙</span></span>
                &lt; 80% 铺货不足，按规则不参与主推。
              </span>
            </div>
          )}
                  ...
                  <div className="p-3 flex flex-col gap-1.5 flex-1">
                    ...
                    {item.coverage > 0 && (
                      <div
                        ...
                        title={`铺店覆盖率 ${Math.round(item.coverage * 100)}%（该款铺店数 ÷ 品牌铺店总数）${item.coverage >= 0.8 ? '——已达标推荐' : '——不足 80%，未达主推标准'}`}
                      >
...
// Compare two ISO-style week strings like "26W20" / "26W21" / "25W52".
