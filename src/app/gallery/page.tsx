  ...
  return (
    <div
      className={`relative group rounded-xl overflow-hidden border-2 transition-all cursor-pointer ${
        selected ? 'border-indigo-500 ring-2 ring-indigo-400/50' : 'border-zinc-700 hover:border-zinc-500'
      }`}
    ...
    >
      ...
      >
        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
          selected ? 'bg-indigo-500 border-indigo-500' : 'bg-black/40 border-white/60 group-hover:border-white'
        }`}>
      ...
      <span className={`absolute top-2 right-2 text-xs px-1.5 py-0.5 rounded font-medium ${
        item.source === 'tryon' ? 'bg-indigo-600 text-white' : item.source === 'image-design' ? 'bg-violet-600 text-white' : 'bg-emerald-600 text-white'
      }`}>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent px-2 py-2">
        ...
        <p className="text-zinc-400 text-[10px]">{new Date(item.createdAt).toLocaleDateString('zh-CN')}</p>
      </div>
    </div>
  )
}
...
function BatchLogoModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-zinc-800 rounded-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-zinc-700 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold text-white">批量添加 Logo</h3>
            <p className="text-sm text-zinc-400 mt-0.5">将对 {count} 张图片统一添加 Logo</p>
          </div>
          <button onClick={onClose} className="text-zinc-400 hover:text-white text-2xl w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-700">×</button>
        </div>

        <div className="p-6">
          <p className="text-sm text-zinc-400 mb-3">拖动下方预览框设置 Logo 位置，所有图片将使用相同位置</p>

          {/* 位置预览框 */}
          <div
            ref={previewRef}
            className="relative w-full aspect-[3/4] bg-zinc-700 rounded-xl overflow-hidden cursor-crosshair border border-zinc-600 select-none"
        ...
        <div className="flex gap-3 px-6 pb-6">
          <button onClick={onClose} className="flex-1 py-2.5 bg-zinc-700 hover:bg-zinc-600 text-white text-sm font-medium rounded-lg transition-colors">取消</button>
          <button
...
function PreviewModal({ item, onClose }: { item: GalleryItem; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-zinc-800 rounded-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-700 shrink-0">
          <div>
            ...
            <p className="text-zinc-400 text-xs mt-0.5">{new Date(item.createdAt).toLocaleString('zh-CN')}</p>
          </div>
          <button onClick={onClose} className="text-zinc-400 hover:text-white text-2xl w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-700">×</button>
        </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-zinc-700 shrink-0">
          <button onClick={handleDownload} className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors">下载</button>
          <button onClick={onClose} className="px-4 py-2.5 bg-zinc-700 hover:bg-zinc-600 text-white text-sm font-medium rounded-lg transition-colors">关闭</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-zinc-900 text-white">
      ...
      <header className="border-b border-zinc-800 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400">我的图库</p>
          </div>
        </div>
        {user && (
          <div className="flex items-center gap-3">
            ...
            <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-sm font-bold">{user.name[0]}</div>
      ...
      <nav className="border-b border-zinc-800 px-6 flex gap-1">
        {NAV.map(item => (
          <a key={item.label} href={item.href} className={`flex items-center gap-1.5 px-4 py-3 text-sm font-medium border-b-2 transition-all ${
            item.active ? 'border-indigo-500 text-white' : 'border-transparent text-zinc-400 hover:text-white hover:border-zinc-600'
          }`}>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-white">我的图库</h2>
            <p className="text-sm text-zinc-400 mt-0.5">共 {items.length} 张图片 · 来自 AI 换装、模板社区和 AI 图片设计的下载记录</p>
          </div>
          {items.length > 0 && (
        ...
        {selectedCount > 0 && (
          <div className="flex items-center gap-3 mb-5 px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-xl">
            <span className="text-sm text-zinc-300 font-medium">已选 {selectedCount} 张</span>
            ...
            <button
              ...
              className="px-4 py-1.5 bg-zinc-700 hover:bg-zinc-600 disabled:opacity-40 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-2"
            >
            ...
            <button onClick={clearSelection} className="text-zinc-500 hover:text-zinc-300 text-sm px-2">取消</button>
          </div>
        )}

        {/* 批量处理进度 */}
        {batchProcessing && (
          <div className="mb-5 px-4 py-3 bg-indigo-900/30 border border-indigo-700/40 rounded-xl flex items-center gap-3">
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 text-zinc-500">
            ...
            <p className="text-lg font-medium text-zinc-400">图库还是空的</p>
            <p className="text-sm mt-2">在 AI 换装、模板社区或 AI 图片设计下载图片后，会自动出现在这里</p>
            <div className="flex gap-3 mt-6">
              <a href="/tryon" className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium rounded-lg transition-colors">去 AI 换装</a>
              <a href="/templates" className="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 text-white text-sm font-medium rounded-lg transition-colors">去模板社区</a>
              <a href="/image-design" className="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 text-white text-sm font-medium rounded-lg transition-colors">去 AI 图片设计</a>
            </div>
          </div>
        ) : (
